const ingresosAnuales = parseFloat(
    prompt("Ingrese los ingresos brutos anuales:")
);

const superficie = parseFloat(
    prompt("Ingrese la superficie afectada en m²:")
);

if (isNaN(ingresosAnuales) || isNaN(superficie)) {

    alert(" Debe ingresar valores numéricos válidos.");

} else {

    let categoria;

    if (ingresosAnuales <= 6000000 && superficie <= 30) {

        categoria = "Categoría A";

    } else if (ingresosAnuales <= 12000000 && superficie <= 45) {

        categoria = "Categoría B";

    } else if (ingresosAnuales <= 18000000 && superficie <= 85) {

        categoria = "Categoría C";

    } else {

        categoria = "Régimen General";
    }

    alert("Categoría asignada: " + categoria);
}